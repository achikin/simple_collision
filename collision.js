...
function belongs_to(point, line) {
    line_start_x = Math.min(line.x1, line.x2);
    return (point.x > Math.min(line.x1, line.x2) && point.x < Math.max(line.x1, line.x2) && point.y > Math.min(line.y1, line.y2) && point.y < Math.max(line.y1, line.y2));
}

function collide_lines(line1, line2) {
    var A1 = line1.e.y - line1.s.y;
    var B1 = line1.s.x - line1.e.x;
    var C1 = line1.e.x*line1.s.y - line1.s.x*line1.e.y;
    var A2 = line2.e.y - line2.s.y;
    var B2 = line2.s.x - line2.e.x;
    var C2 = line2.e.x*line2.s.y - line2.s.x*line2.e.y;
    var denom = B2*A1 - B1 * A2;
    if (denom == 0) {
        return false;
    }
    var x = (C2 * B1 - C1*B2)/ denom;
    var y = (C1* A2 - C2*A1) / denom;
